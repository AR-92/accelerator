...
const routerNode = async (state) => {
  ...
  const query =
    lastMessage?.content?.toLowerCase() || state.query.toLowerCase();
...
const processNode = async (state) => {
  try {
    ...
    const prompt = ChatPromptTemplate.fromMessages([
      [
        'system',
        `You are an AI assistant. Use the context provided to answer questions accurately. 
    ...
    logger.error('Error in processNode:', error);
    return {
      ...state,
      error: error.message,
      currentStep: 'error',
...
const createAgentGraph = () => {
  const workflow = new StateGraph(AgentState)
    ...
    .addConditionalEdges('router', (state) => state.currentStep, {
