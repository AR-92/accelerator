 ...
 * @returns {Promise<{success: boolean, title: string, category: string, description: string, tags: string[], error: any}>}
 */
export async function generateRandomIdea(prompt = "") {
  let systemPrompt = `Generate a creative startup idea. Return a JSON object with exactly these fields:
- title: A compelling 3-8 word title
- category: One category from: Technology, Healthcare, Finance, Education, E-commerce, Entertainment, Social, Environment, or Other
...
Be innovative and unexpected. Return only valid JSON, no other text.`;
  ...
  if (prompt && prompt.trim()) {
    systemPrompt = `Generate a creative startup idea inspired by: "${prompt}". Return a JSON object with exactly these fields:
- title: A compelling 3-8 word title
- category: One category from: Technology, Healthcare, Finance, Education, E-commerce, Entertainment, Social, Environment, or Other
- description: A detailed description (100-2000 characters) inspired by the user's request
- tags: Array of 3-5 relevant tags (lowercase, hyphen-separated)

Be innovative and tailor the idea to match the inspiration provided. Return only valid JSON, no other text.`;
  }
 ...
 */
export async function chatAboutRequirements(messages, ideaId = null) {
