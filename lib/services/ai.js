...
export async function generateDescriptionForSection(
  ...
  try {
    ...
    // Simple template replacement (for now, replace {{question}} with input)
    let systemPrompt = template.replace(/\{\{question\}\}/g, input);
...
export async function improveDescriptionForSection(
  ...
  try {
    ...
    let systemPrompt = template.replace(/\{\{description\}\}/g, description);
