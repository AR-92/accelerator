...
import { requirePackage, freePackageLimits } from "../../middleware/package.js";
...
router.get("/portfolio", requireAuth, async (req, res) => {
  ...
  const { data: ideas, error } = await supabase
    .from("ideas")
    .select(
      "id, title, description, tags, category, rating, is_favorite, privacy, completion_percentage, overall_status, created_at, updated_at",
    )
    .eq("user_id", req.user.id)
    .order("updated_at", { ascending: false })
    .order("created_at", { ascending: false });
...
// Explore Ideas page
router.get("/explore-idea", async (req, res) => {
  const { createClient } = await import("@supabase/supabase-js");
  const config = (await import("../../config.js")).default;
  const supabase = createClient(config.supabase.url, config.supabase.key);
