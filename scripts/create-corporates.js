/**
 * Script to create 10 sample corporate entries using the CorporateService
 */
...
async function createCorporates() {
  try {
    console.log('Starting corporate creation...');

    const corporateService = container.get('corporateService');

    for (const corporateData of corporates) {
    ...
    console.log('All corporates created successfully!');
  } catch (error) {
    console.error('Error creating corporates:', error);
  } finally {
    process.exit(0);
  }
}
