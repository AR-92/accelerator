...
class AdminUserActionController {
  ...
  /**
   * Stop impersonating user (page redirect)
   * @param {Object} req - Express request object
   * @param {Object} res - Express response object
   */
  async stopImpersonateUserPage(req, res) {
    try {
      // Check if we are currently impersonating
      if (!req.session.originalUser || !req.session.originalUserId) {
        return res.status(400).render('pages/error/page-not-found', {
          title: 'Error - Admin Panel',
          layout: 'admin',
