...
export const getContent = async (req, res) => {
  try {
    ...
    logger.info(`Fetched ${content ? content.length : 0} content items, total count: ${count}`);
    ...
    logger.info(`Fetched ${content.length} of ${total} content items${filters.length ? ` (filtered by ${filters.join(', ')})` : ''}`);
