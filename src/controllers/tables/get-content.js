...
export const getContent = async (req, res) => {
  try {
    ...
    logger.info(
      `Fetched ${content ? content.length : 0} content items, total count: ${count}`
    );
    ...
    logger.info(
      `Fetched ${content.length} of ${total} content items${filters.length ? ` (filtered by ${filters.join(', ')})` : ''}`
    );
    ...
    const colspan =
      contentColumns.length + (true ? 1 : 0) + (actions.length > 0 ? 1 : 0);
    ...
    if (isHtmxRequest(req)) {
          ...
          <thead class="bg-card border-b border-border">
            <tr>
              <th class="px-6 py-4 text-left font-semibold text-card-foreground uppercase text-xs tracking-wider bg-muted">
                <input type="checkbox" id="selectAll-content" class="rounded border-input text-primary">
              </th>`;
      ...
      if (mappedContent.length > 0) {
        mappedContent.forEach((item) => {
          tableHtml += `<tr id="content-row-${item.id}" class="h-16 border-b border-border hover:bg-muted/50 even:bg-muted/30 transition-colors duration-150">
            <td class="px-6 py-4">
              <input type="checkbox" class="contentCheckbox rounded border-input text-primary value="${item.id}" data-content-id="${item.id}">
            </td>`;
          ...
          contentColumns.forEach((column) => {
            ...
            if (column.type === 'status') {
              cellContent = `<span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${item.status === 'published' ? 'bg-green-100 text-green-800' : item.status === 'draft' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800'}">${item[column.key]}</span>`;
          ...
          if (actions.length > 0) {
            tableHtml += `<td class="px-6 py-4">
              <div class="relative">
                <button onclick="toggleActionMenu('content', ${item.id})" class="p-2 rounded-full hover:bg-accent text-muted-foreground hover:text-accent-foreground transition-colors">
                ...
                <div id="actionMenu-content-${item.id}" class="dropdown-menu hidden absolute right-0 mt-2 w-48 bg-popover rounded-md shadow-lg z-10 border border-border">
                  <div class="py-1">`;
            ...
            actions.forEach((action) => {
              if (action.type === 'link') {
                ...
                tableHtml += `<button onclick="${action.onclick}(${item.id})" class="flex items-center w-full px-4 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors">
                  ${action.icon}
                  ${action.label}
                </button>`;
              } else if (action.type === 'delete') {
                tableHtml += `<button onclick="${action.onclick}(${item.id}, '${item.title}')" class="flex items-center w-full px-4 py-2 text-sm text-destructive hover:bg-destructive/10 transition-colors">
                  ${action.icon}
                  ${action.label}
                </button>`;
              }
        ...
        tableHtml += `<tr class="h-16">
          <td colspan="${contentColumns.length + 2}" class="px-6 py-8 text-center text-muted-foreground">
            <div class="flex flex-col items-center justify-center py-12">
              ...
              <p class="text-lg font-medium text-muted-foreground mb-2">No content found</p>
              <p class="text-sm text-muted-foreground/70">Get started by creating your first content item.</p>
            </div>
          </td>
        </tr>`;
      }
      ...
      tableHtml += `
          ...
          <tfoot>
            <tr>
              <td colspan="${contentColumns.length + 2}" class="bg-card border-t border-border p-4">
                <div class="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div class="text-sm text-muted-foreground">
                    Showing ${start}-${end} of ${total} content items
                  </div>

                  <div class="flex items-center gap-3">
                    <span class="text-sm text-muted-foreground font-medium">Rows per page:</span>
                      <select id="rowsPerPage-content" name="limit" class="border border-input rounded-lg px-3 py-2 text-sm bg-background shadow-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                        hx-get="/admin/table-pages/content" hx-target="#contentTableContainer" hx-include="[name='search'],[name='status']" hx-vals="js:{page: 1, limit: document.getElementById('rowsPerPage-content').value}">
      ...
      if (hasPrev) {
        tableHtml += `<button hx-get="/admin/table-pages/content?page=${prevPage}${queryString}" hx-target="#contentTableContainer"
      ...
      pages.forEach((page) => {
        ...
        tableHtml += `<button hx-get="/admin/table-pages/content?page=${page}${queryString}" hx-target="#contentTableContainer"
      ...
      if (hasNext) {
        tableHtml += `<button hx-get="/admin/table-pages/content?page=${nextPage}${queryString}" hx-target="#contentTableContainer"
      ...
      if (true && bulkActions.length > 0) {
        tableHtml += `
        <div id="bulkActions-content" class="fixed bottom-20 left-1/2 transform -translate-x-1/2 bg-primary text-primary-foreground px-6 py-3 rounded-full z-30" style="display: none;">
          <div class="flex items-center gap-4">
            <span id="selectedCount-content">0 content items selected</span>
            <div class="flex gap-2">`;

        bulkActions.forEach((action) => {
          tableHtml += `<button onclick="${action.onclick}" id="${action.buttonId}-content" class="bg-primary-foreground/20 hover:bg-primary-foreground/30 text-primary-foreground px-3 py-1 rounded text-sm" disabled="">
            ${action.label}
          </button>`;
        });
        ...
        <table class="min-w-full table-auto bg-card">
          <thead class="bg-card border-b border-border">
            <tr>
              ...
              <th class="px-6 py-4 text-left font-semibold text-card-foreground uppercase text-xs tracking-wider bg-muted">Description</th>
              <th class="px-6 py-4 text-left font-semibold text-card-foreground uppercase text-xs tracking-wider bg-muted">Type</th>
            ...
            <tr class="h-16">
              <td colspan="7" class="px-6 py-8 text-center text-muted-foreground">
                <div class="flex flex-col items-center justify-center py-12">
                  ...
                  <p class="text-lg font-medium text-muted-foreground mb-2">Error loading content</p>
          ...
          <tfoot>
            <tr>
              <td colspan="7" class="bg-card border-t border-border p-4">
                <div class="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div class="text-sm text-muted-foreground">Showing 0-0 of 0 content items</div>
                  <div class="flex items-center gap-3">
                    <span class="text-sm text-muted-foreground font-medium">Rows per page:</span>
                    <select id="rowsPerPage-content" name="limit" class="border border-input rounded-lg px-3 py-2 text-sm bg-background shadow-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent">
