...
export const getLearningContent = async (req, res) => {
  try {
    ...
    logger.info(
      `Fetched ${learningContents ? learningContents.length : 0} learning contents, total count: ${count}`
    );
    ...
    logger.info(
      `Fetched ${learningContents.length} of ${total} learning contents${filters.length ? ` (filtered by ${filters.join(', ')})` : ''}`
    );
    ...
    const actions = [
      {
        type: 'link',
        url: '/admin/other-pages/learning-content',
        label: 'View Details',
