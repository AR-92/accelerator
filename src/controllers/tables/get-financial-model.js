...
export const getFinancialModel = async (req, res) => {
  try {
    ...
    query = applyTableFilters(query, 'financial-model', req.query);
    ...
    const { data: financialModels, error, count } = await query;
    ...
    logger.info(`Fetched ${financialModels ? financialModels.length : 0} financial models, total count: ${count}`);
    ...
    logger.info(`Fetched ${financialModels.length} of ${total} financial models${filters.length ? ` (filtered by ${filters.join(', ')})` : ''}`);
    ...
    const actions = [
      {
        type: 'link',
        url: '/admin/other-pages/financial-model',
        label: 'View Details',
    ...
    const bulkActions = [
      { onclick: 'bulkFinalizeModels', buttonId: 'bulkFinalizeBtn', label: 'Mark as Final' },
      { onclick: 'bulkArchiveModels', buttonId: 'bulkArchiveBtn', label: 'Archive Selected' },
      { onclick: 'bulkDeleteModels', buttonId: 'bulkDeleteBtn', label: 'Delete Selected' }
    ];
    ...
    const statusCounts = await getStatusCounts('financial-model', databaseService);
    const filterCounts = getFilterCounts('financial-model', statusCounts);
    const tableConfig = getTableConfig('financial-model');

    // Make variables available to layout for filter-nav
    res.locals.tableConfig = tableConfig;
    res.locals.filterCounts = filterCounts;
    res.locals.currentPage = 'financial-model';
    ...
    if (isHtmxRequest(req)) {
      ...
      const tableHtml = `
        <table class="min-w-full table-auto bg-card">
          ...
          <thead class="bg-card border-b border-border">
            <tr>
              <th class="px-6 py-4 text-left font-semibold text-card-foreground uppercase text-xs tracking-wider bg-muted">
                <input type="checkbox" id="selectAll-financial-model" class="rounded border-input text-primary" aria-label="Select all financial models">
              </th>
          ...
          <tbody class="text-sm text-card-foreground">
            ${financialModels.length > 0 ? financialModels.map(model => `
              <tr id="financial-model-row-${model.id}" class="h-16 border-b border-border hover:bg-muted/50 even:bg-muted/30 transition-colors duration-150">
                <td class="px-6 py-4">
                  <input type="checkbox" class="financial-modelCheckbox rounded border-input text-primary value="${model.id}" data-financial-model-id="${model.id}" aria-label="Select financial model ${model.model_name}">
                </td>
                <td class="px-6 py-4">
                  <div class="text-sm text-card-foreground truncate max-w-xs" title="${model.model_name}">${model.model_name}</div>
                </td>
                <td class="px-6 py-4">
                  <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${model.model_status === 'final' ? 'bg-green-100 text-green-800' : model.model_status === 'draft' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800'}">${model.model_status}</span>
                </td>
                <td class="px-6 py-4">
                  <div class="text-sm text-card-foreground truncate max-w-xs" title="${model.progress_percentage}%">${model.progress_percentage}%</div>
                </td>
                <td class="px-6 py-4">
                  <div class="text-sm text-card-foreground truncate max-w-xs" title="$${model.monthly_revenue}">$${model.monthly_revenue}</div>
                </td>
                <td class="px-6 py-4 hidden lg:table-cell">
                  <div class="text-sm text-card-foreground">${new Date(model.created_at).toLocaleDateString()}</div>
                </td>
                <td class="px-6 py-4">
                  <div class="relative">
                    <button onclick="toggleActionMenu('financial-model', ${model.id})" class="p-2 rounded-full hover:bg-accent text-muted-foreground hover:text-accent-foreground transition-colors" aria-label="Actions menu">
                    ...
                    <div id="actionMenu-financial-model-${model.id}" class="dropdown-menu hidden absolute right-0 mt-2 w-48 bg-popover rounded-md shadow-lg z-10 border border-border">
                      <div class="py-1">
                        <a href="/admin/other-pages/financial-model" class="flex items-center px-4 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors">
                          <svg class="w-4 h-4 mr-3 lucide lucide-eye" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2.062 12.348a1 1 0 0 1 0-.696 10.75 10.75 0 0 1 19.876 0 1 1 0 0 1 0 .696 10.75 10.75 0 0 1-19.876 0"></path><circle cx="12" cy="12" r="3"></circle></svg>
                          View Details
                        </a>
                        <button onclick="editFinancialModel(${model.id})" class="flex items-center w-full px-4 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors">
                        ...
                        <button onclick="deleteFinancialModel(${model.id}, '${model.model_name}')" class="flex items-center w-full px-4 py-2 text-sm text-destructive hover:bg-destructive/10 transition-colors">
              ...
              <tr class="h-16">
                <td colspan="${colspan}" class="px-6 py-8 text-center text-muted-foreground">
                  <div class="flex flex-col items-center justify-center py-12">
                    ...
                    <p class="text-lg font-medium text-muted-foreground mb-2">No financial models found</p>
                    <p class="text-sm text-muted-foreground/70">Get started by creating your first financial model.</p>
                  </div>
                </td>
              </tr>
            `}
              ...
              <td colspan="${colspan}" class="bg-card border-t border-border p-4">
                <div class="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div class="text-sm text-muted-foreground">
                    Showing ${start}-${end} of ${total} financial models
                  </div>
                  <div class="flex items-center gap-3">
                    <span class="text-sm text-muted-foreground font-medium">Rows per page:</span>
                    <select id="rowsPerPage-financial-model" name="limit" class="border border-input rounded-lg px-3 py-2 text-sm bg-background shadow-sm focus:outline-none focus:border-ring transition-colors" hx-get="/admin/table-pages/financial-model" hx-target="#financial-modelTableContainer" hx-include="[name='search'],[name='status']">
      ...
      res.render('admin/table-pages/financial-model', {
        ...
        currentUrl: '/admin/table-pages/financial-model',
        colspan,
        filterCounts,
        tableConfig
      });
