...
export const getFinancialModel = async (req, res) => {
  try {
    ...
    logger.info(
      `Fetched ${financialModels ? financialModels.length : 0} financial models, total count: ${count}`
    );
    ...
    logger.info(
      `Fetched ${financialModels.length} of ${total} financial models${filters.length ? ` (filtered by ${filters.join(', ')})` : ''}`
    );
