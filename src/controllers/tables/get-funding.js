...
export const getFunding = async (req, res) => {
  try {
    ...
    logger.info(`Fetched ${fundings ? fundings.length : 0} fundings, total count: ${count}`);
    ...
    logger.info(`Fetched ${fundings.length} of ${total} fundings${filters.length ? ` (filtered by ${filters.join(', ')})` : ''}`);
    ...
    const bulkActions = [
      { onclick: 'bulkFundFundings', buttonId: 'bulkFundBtn', label: 'Mark as Funded' },
      { onclick: 'bulkCloseFundings', buttonId: 'bulkCloseBtn', label: 'Close Selected' },
      { onclick: 'bulkDeleteFundings', buttonId: 'bulkDeleteBtn', label: 'Delete Selected' }
    ];
