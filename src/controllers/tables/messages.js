...
export const getMessages = async (req, res) => {
  try {
    ...
    logger.info(`Fetched ${messages ? messages.length : 0} messages, total count: ${count}`);
    ...
    logger.info(`Fetched ${messages.length} of ${total} messages${filters.length ? ` (filtered by ${filters.join(', ')})` : ''}`);
