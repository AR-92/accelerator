...
export const getBusinessPlan = async (req, res) => {
  try {
    ...
    query = applyTableFilters(query, 'business-plan', req.query);
    ...
    const { data: businessPlans, error, count } = await query;
    ...
    logger.info(`Fetched ${businessPlans ? businessPlans.length : 0} business plans, total count: ${count}`);
    ...
    const statusCounts = await getStatusCounts('business-plan', databaseService);
    logger.info(`Status counts: ${JSON.stringify(statusCounts)}`);

    const tableConfig = getTableConfig('business-plan');
    ...
    if (isHtmxRequest(req)) {
          ...
          <thead class="bg-card border-b border-border">
            <tr>
              <th class="px-6 py-4 text-left font-semibold text-card-foreground uppercase text-xs tracking-wider bg-muted">
                <input type="checkbox" id="selectAll-business-plan" class="rounded border-input text-primary">
              </th>`;
      ...
      if (businessPlans.length > 0) {
        businessPlans.forEach(plan => {
          tableHtml += `<tr id="business-plan-row-${plan.id}" class="h-16 border-b border-border hover:bg-muted/50 even:bg-muted/30 transition-colors duration-150">
            <td class="px-6 py-4">
              <input type="checkbox" class="business-planCheckbox rounded border-input text-primary value="${plan.id}" data-business-plan-id="${plan.id}">
            </td>`;
          ...
          columns.forEach(column => {
            ...
            if (column.type === 'status') {
              cellContent = `<span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${plan.status === 'approved' ? 'bg-green-100 text-green-800' : plan.status === 'review' ? 'bg-yellow-100 text-yellow-800' : plan.status === 'draft' ? 'bg-gray-100 text-gray-800' : 'bg-gray-100 text-gray-800'}">${plan[column.key]}</span>`;
            } else if (column.type === 'date') {
              cellContent = `<div class="text-sm text-card-foreground">${new Date(plan[column.key]).toLocaleDateString()}</div>`;
            } else {
              cellContent = `<div class="text-sm text-card-foreground truncate max-w-xs" title="${plan[column.key]}">${plan[column.key]}</div>`;
            }
          ...
          if (actions.length > 0) {
            tableHtml += `<td class="px-6 py-4">
              <div class="relative">
                <button onclick="toggleActionMenu('business-plan', ${plan.id})" class="p-2 rounded-full hover:bg-accent text-muted-foreground hover:text-accent-foreground transition-colors">
                ...
                <div id="actionMenu-business-plan-${plan.id}" class="dropdown-menu hidden absolute right-0 mt-2 w-48 bg-popover rounded-md shadow-lg z-10 border border-border">
                  <div class="py-1">`;

            actions.forEach(action => {
              if (action.type === 'link') {
                tableHtml += `<a href="${action.url}/${plan.id}" class="flex items-center px-4 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors">
                  ${action.icon}
                  ${action.label}
                </a>`;
              } else if (action.type === 'button') {
                tableHtml += `<button onclick="${action.onclick}(${plan.id})" class="flex items-center w-full px-4 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors">
                  ${action.icon}
                  ${action.label}
                </button>`;
              } else if (action.type === 'delete') {
                tableHtml += `<button onclick="${action.onclick}(${plan.id})" class="flex items-center w-full px-4 py-2 text-sm text-red-600 hover:bg-red-50 hover:text-red-700 transition-colors">
                  ${action.icon}
                  ${action.label}
                </button>`;
              }
        ...
        tableHtml += `<tr class="h-16">
          <td colspan="${colspan}" class="px-6 py-8 text-center text-muted-foreground">
            <div class="flex flex-col items-center justify-center py-12">
              ...
              <p class="text-lg font-medium text-muted-foreground mb-2">No business plans found</p>
              <p class="text-sm text-muted-foreground/70">Business plans will appear here.</p>
            </div>
          </td>
        </tr>`;
      }
              ...
              <td colspan="${colspan}" class="bg-card border-t border-border p-4">
                <div class="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div class="text-sm text-muted-foreground">
                    Showing ${start}-${end} of ${total} business plans
                  </div>

                  <div class="flex items-center gap-3">
                    <span class="text-sm text-muted-foreground font-medium">Rows per page:</span>
                      <select id="rowsPerPage-business-plan" name="limit" class="border border-input rounded-lg px-3 py-2 text-sm bg-background shadow-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                        hx-get="/admin/table-pages/business-plan" hx-target="#business-planTableContainer" hx-vals="js:{limit: document.getElementById('rowsPerPage-business-plan').value}">
      ...
      if (hasPrev) {
        tableHtml += `<button hx-get="/admin/table-pages/business-plan?page=${prevPage}" hx-target="#business-planTableContainer"
      ...
      pages.forEach(page => {
        ...
        tableHtml += `<button hx-get="/admin/table-pages/business-plan?page=${page}" hx-target="#business-planTableContainer"
      ...
      if (hasNext) {
        tableHtml += `<button hx-get="/admin/table-pages/business-plan?page=${nextPage}" hx-target="#business-planTableContainer"
      ...
      res.render('admin/table-pages/business-plan', {
        title: 'Business Plan Management',
        currentPage: 'business-plan',
        currentSection: 'business',
        isTablePage: true,
        tableId: 'business-plan',
        ...
        statusCounts,
        currentUrl: '/admin/table-pages/business-plan',
        colspan,
        filterCounts,
        tableConfig
      });
    ...
    res.render('admin/table-pages/business-plan', {
      ...
      tableConfig: getTableConfig('business-plan')
    });
  }
};