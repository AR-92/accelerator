...
export const getEnterprises = async (req, res) => {
  try {
    ...
    query = applyTableFilters(query, 'enterprises', req.query);
    ...
    const { data: enterprises, error, count } = await query;
    ...
    logger.info(`Fetched ${enterprises ? enterprises.length : 0} enterprises, total count: ${count}`);

    const total = count || 0;

    // Get status counts using the dynamic helper
    const statusCounts = await getStatusCounts('enterprises', databaseService);
    ...
    logger.info(`Fetched ${enterprises.length} of ${total} enterprises${filters.length ? ` (filtered by ${filters.join(', ')})` : ''}`);
    ...
    const bulkActions = [
      { onclick: 'bulkDeleteEnterprises', buttonId: 'bulkDeleteBtn', label: 'Delete Selected' }
    ];
    ...
    const filterCounts = getFilterCounts('enterprises', statusCounts);
    const tableConfig = getTableConfig('enterprises');

    // Make variables available to layout for filter-nav
    res.locals.tableConfig = tableConfig;
    res.locals.filterCounts = filterCounts;
    res.locals.currentPage = 'enterprises';
    ...
    if (isHtmxRequest(req)) {
          ...
          <thead class="bg-card border-b border-border">
            <tr>
              <th class="px-6 py-4 text-left font-semibold text-card-foreground uppercase text-xs tracking-wider bg-muted">
                <input type="checkbox" id="selectAll-enterprises" class="rounded border-input text-primary">
              </th>`;
      ...
      if (enterprises.length > 0) {
        enterprises.forEach(enterprise => {
          tableHtml += `<tr id="enterprise-row-${enterprise.id}" class="h-16 border-b border-border hover:bg-muted/50 even:bg-muted/30 transition-colors duration-150">
            <td class="px-6 py-4">
              <input type="checkbox" class="enterpriseCheckbox rounded border-input text-primary value="${enterprise.id}" data-enterprise-id="${enterprise.id}">
            </td>`;
          ...
          columns.forEach(column => {
            ...
            if (column.type === 'status') {
              cellContent = `<span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${enterprise.status === 'active' ? 'bg-green-100 text-green-800' : enterprise.status === 'inactive' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'}">${enterprise[column.key]}</span>`;
            } else {
              cellContent = `<div class="text-sm text-card-foreground truncate max-w-xs" title="${enterprise[column.key]}">${enterprise[column.key]}</div>`;
            }
          ...
          if (actions.length > 0) {
            tableHtml += `<td class="px-6 py-4">
              <div class="relative">
                <button onclick="toggleActionMenu('enterprise', ${enterprise.id})" class="p-2 rounded-full hover:bg-accent text-muted-foreground hover:text-accent-foreground transition-colors">
                ...
                <div id="actionMenu-enterprise-${enterprise.id}" class="dropdown-menu hidden absolute right-0 mt-2 w-48 bg-popover rounded-md shadow-lg z-10 border border-border">
                  <div class="py-1">`;

            actions.forEach(action => {
              if (action.type === 'link') {
                tableHtml += `<a href="${action.url}/${enterprise.id}" class="flex items-center px-4 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors">
                  ${action.icon}
                  ${action.label}
                </a>`;
              } else if (action.type === 'button') {
                tableHtml += `<button onclick="${action.onclick}(${enterprise.id})" class="flex items-center w-full px-4 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors">
                  ${action.icon}
                  ${action.label}
                </button>`;
              } else if (action.type === 'delete') {
                tableHtml += `<button onclick="${action.onclick}(${enterprise.id}, '${enterprise.name}')" class="flex items-center w-full px-4 py-2 text-sm text-destructive hover:bg-destructive/10 transition-colors">
                  ${action.icon}
                  ${action.label}
                </button>`;
              }
        ...
        tableHtml += `<tr class="h-16">
          <td colspan="${colspan}" class="px-6 py-8 text-center text-muted-foreground">
            <div class="flex flex-col items-center justify-center py-12">
              ...
              <p class="text-lg font-medium text-muted-foreground mb-2">No enterprises found</p>
              <p class="text-sm text-muted-foreground/70">Get started by creating your first enterprise.</p>
            </div>
          </td>
        </tr>`;
      }
      ...
      tableHtml += `
                ...
                <div class="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  <div class="flex items-center gap-3">
                    <span class="text-sm text-muted-foreground font-medium">Rows per page:</span>
                      <select id="rowsPerPage-enterprises" name="limit" class="border border-input rounded-lg px-3 py-2 text-sm bg-background shadow-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                        hx-get="/admin/table-pages/enterprises" hx-target="#enterprisesTableContainer" hx-vals="js:{limit: document.getElementById('rowsPerPage-enterprises').value}">
        ...
        if (hasPrev) {
           tableHtml += `<button hx-get="/admin/table-pages/enterprises?page=${prevPage}" hx-target="#enterprisesTableContainer"
        ...
        pages.forEach(page => {
          const isActive = page === pageNum;
           tableHtml += `<button hx-get="/admin/table-pages/enterprises?page=${page}" hx-target="#enterprisesTableContainer"
        ...
        if (hasNext) {
           tableHtml += `<button hx-get="/admin/table-pages/enterprises?page=${nextPage}" hx-target="#enterprisesTableContainer"
      ...
      if (true && bulkActions.length > 0) {
        tableHtml += `
        <div id="bulkActions-enterprises" class="fixed bottom-20 left-1/2 transform -translate-x-1/2 bg-primary text-primary-foreground px-6 py-3 rounded-full z-30" style="display: none;">
          <div class="flex items-center gap-4">
            <span id="selectedCount-enterprises">0 enterprises selected</span>
            <div class="flex gap-2">`;

        bulkActions.forEach(action => {
          tableHtml += `<button onclick="${action.onclick}" id="${action.buttonId}-enterprises" class="bg-primary-foreground/20 hover:bg-primary-foreground/30 text-primary-foreground px-3 py-1 rounded text-sm" disabled="">
            ${action.label}
          </button>`;
        });
      ...
      res.render('admin/table-pages/enterprises', {
        ...
        currentUrl: '/admin/table-pages/enterprises',
        colspan,
        filterCounts,
        tableConfig
      });
    ...
    res.render('admin/table-pages/enterprises', {
      ...
      query: { search: '', status: '' },
      filterCounts: { all: 0, active: 0, inactive: 0, acquired: 0 }
    });
  }
};