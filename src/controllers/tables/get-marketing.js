...
export const getMarketing = async (req, res) => {
  try {
    ...
    logger.info(
      `Fetched ${marketings ? marketings.length : 0} marketings, total count: ${count}`
    );
    ...
    logger.info(
      `Fetched ${marketings.length} of ${total} marketings${filters.length ? ` (filtered by ${filters.join(', ')})` : ''}`
    );
