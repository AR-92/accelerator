...
export const getLearningCategories = async (req, res) => {
  try {
    ...
    logger.info(
      `Query params: search="${search}", page=${page}, limit=${limit}`
    );
    ...
    logger.info(
      `Fetched ${learningCategories ? learningCategories.length : 0} learning categories, total count: ${count}`
    );
    ...
    logger.info(
      `Fetched ${learningCategories.length} of ${total} learning categories${filters.length ? ` (filtered by ${filters.join(', ')})` : ''}`
    );
    ...
    const actions = [
      {
        type: 'link',
        url: '/admin/other-pages/learning-categories',
        label: 'View Details',
