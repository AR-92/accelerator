...
export const getHelpCenter = async (req, res) => {
  try {
    ...
    logger.info(`Fetched ${helpCenter ? helpCenter.length : 0} help articles, total count: ${count}`);
    ...
    const bulkActions = [
      { onclick: 'bulkPublishArticles', buttonId: 'bulkPublishBtn', label: 'Publish Selected' },
      { onclick: 'bulkArchiveArticles', buttonId: 'bulkArchiveBtn', label: 'Archive Selected' },
      { onclick: 'bulkDeleteArticles', buttonId: 'bulkDeleteBtn', label: 'Delete Selected' }
    ];
