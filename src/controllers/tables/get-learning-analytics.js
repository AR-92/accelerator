...
export const getLearningAnalytics = async (req, res) => {
  try {
    ...
    const statusCounts = await getStatusCounts('learning-analytics', databaseService);
    ...
    if (isHtmxRequest(req)) {
      res.send('<div class="text-center text-muted-foreground">Error loading learning analytics. Please try again.</div>');
    } else {
      res.render('admin/table-pages/learning-analytics', {
        title: 'Learning Analytics Management',
        currentPage: 'learning-analytics',
