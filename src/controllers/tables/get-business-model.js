...
export const getBusinessModel = async (req, res) => {
  try {
    ...
    logger.info(`Fetched ${businessModels ? businessModels.length : 0} business models, total count: ${count}`);
    ...
    const statusCounts = await getStatusCounts('business-model', databaseService);
