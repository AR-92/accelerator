...
export const getDashboardEnterprise = async (req, res) => {
  try {
    logger.info('Dashboard enterprise section accessed');

    const user = req.user;
    const userRole = user?.role || 'startup';

    // Only allow enterprise or corporate users
    ...
    res.render('pages/dashboard/enterprise', {
