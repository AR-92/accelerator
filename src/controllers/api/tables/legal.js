...
export const getLegal = async (req, res) => {
  try {
    ...
    const { data: legal, error, count } = await query;

    if (error) throw error;

    const total = count || 0;
    logger.info(`Fetched ${legal.length} of ${total} legal items`);
    ...
    if (isHtmxRequest(req)) {
      const legalHtml = legal
        ...
        <tr class="border-b border-gray-100/40 hover:bg-purple-100 dark:hover:bg-purple-800 transition-colors duration-150">
          ...
          <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${item.type}</td>
          <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${item.status || 'N/A'}</td>
          <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${formatDate(item.created_at)}</td>
        </tr>
      `
        )
        .join('');
      ...
      res.send(legalHtml + paginationHtml);
    } else {
      res.json({
        success: true,
        data: legal,
    ...
    logger.error('Error fetching legal:', error);
    if (isHtmxRequest(req)) {
      res.status(500).send('<p class="text-red-500">Error loading legal</p>');
    } else {
...
const generatePaginationHtml = (page, limit, total, query, entity) => {
  ...
  for (let i = startPage; i <= endPage; i++) {
    if (i === page) {
      html += `<span class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground h-9 w-9">${i}</span>`;
    } else {
...
export const bulkAction = async (req, res) => {
  try {
    ...
    const legalService = serviceFactory.getLegalService();
    const results = [];
    const errors = [];
    ...
    for (const id of ids) {
      try {
        ...
        switch (action) {
          case 'approve':
            result = await legalService.approveLegal(id);
            break;
          case 'execute':
            result = await legalService.executeLegal(id);
            break;
          case 'delete':
    ...
    logger.info(
      `Bulk ${action} completed for ${results.length} legal items, ${errors.length} errors`
    );
    ...
    if (isHtmxRequest(req)) {
      ...
      res.send(`
        ...
        <script>
          htmx.ajax('GET', window.location.pathname + window.location.search, {target: '#legalTableContainer'});
      ...
      res.json({
        success: true,
        data: { results, errors },
        message: `Bulk ${action} completed for ${results.length} legal items`,
      });
    }
  } catch (error) {
...
export default function legalRoutes(app) {
  app.get('/api/legal', getLegal);
  app.post('/api/legal/bulk-action', bulkAction);
}
