...
export const getValuation = async (req, res) => {
  try {
    ...
    if (isHtmxRequest(req)) {
      const valuationHtml = valuations
        .map(
          (val) => `
    ...
    if (isHtmxRequest(req)) {
      res
        .status(500)
        .send('<p class="text-red-500">Error loading valuations</p>');
    } else {
...
export const bulkAction = async (req, res) => {
  try {
    ...
    const valuationService = serviceFactory.getValuationService();
    const results = [];
    const errors = [];
    ...
    for (const id of ids) {
      try {
        ...
        switch (action) {
          case 'complete':
            result = await valuationService.completeValuation(id);
            break;
          case 'archive':
            result = await valuationService.archiveValuation(id);
            break;
          case 'delete':
    ...
    logger.info(
      `Bulk ${action} completed for ${results.length} valuations, ${errors.length} errors`
    );
    ...
    if (isHtmxRequest(req)) {
      ...
      res.send(`
        ...
        <script>
          ...
          htmx.trigger('#valuationTableContainer', 'bulkActionCompleted');
        </script>
      `);
    } else {
      res.json({
...
export default function valuationRoutes(app) {
  ...
  app.post('/api/valuation/bulk-action', bulkAction);
}
