...
export const getPackages = async (req, res) => {
  try {
    const { search, package_type, status, page = 1, limit = 10 } = req.query;
    ...
    if (isHtmxRequest(req)) {
      const packageHtml = packages
        ...
        <tr class="border-b border-gray-100/40 hover:bg-purple-100 dark:hover:bg-purple-800 transition-colors duration-150">
          <td class="px-6 py-4">
            <div class="flex items-center">
              ...
              <div class="ml-4">
                ...
                <div class="text-sm text-gray-500">${pkg.description?.substring(0, 50)}${pkg.description?.length > 50 ? '...' : ''}</div>
              </div>
            </div>
          </td>
          ...
          <td class="px-6 py-4 text-sm text-gray-900">${formatDate(pkg.created_at)}</td>
          <td class="px-6 py-4">
            <div class="relative">
              <button onclick="toggleActionMenu('package', ${pkg.id})" class="p-2 rounded-full hover:bg-gray-100 text-gray-500 hover:text-black dark:text-white transition-colors">
              ...
              <div id="actionMenu-package-${pkg.id}" class="dropdown-menu hidden absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-10 border border-gray-200">
                <div class="py-1">
                  <a href="/admin/table-pages/packages/${pkg.id}" class="flex items-center px-4 py-2 text-sm text-black dark:text-white hover:bg-gray-100 transition-colors">
                  ...
                  <button onclick="editPackage(${pkg.id})" class="flex items-center w-full px-4 py-2 text-sm text-black dark:text-white hover:bg-gray-100 transition-colors">
                  ...
                  <button onclick="deletePackage(${pkg.id}, '${pkg.name}')" class="flex items-center w-full px-4 py-2 text-sm text-red-600 hover:bg-red-50 transition-colors">
          ...
          <div class="fixed top-4 right-4 z-50 max-w-sm w-full">
            <div class="relative w-full rounded-lg border px-4 py-3 text-sm bg-green-50 text-green-800 border-green-200">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">Package "${pkg.name}" created successfully!</div>
              </div>
            </div>
          </div>
          <script>
            setTimeout(() => document.querySelector('.fixed').remove(), 3000);
            htmx.trigger('#packagesTableContainer', 'packageCreated');
          </script>
        `;

        res.send(alertHtml + packageHtml);
      } else {
          ...
          <div class="fixed top-4 right-4 z-50 max-w-sm w-full">
            <div class="relative w-full rounded-lg border px-4 py-3 text-sm bg-red-50 text-red-800 border-red-200">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">Failed to create package: ${error.message}</div>
              </div>
            </div>
          </div>
        `);
          ...
          <div class="fixed top-4 right-4 z-50 max-w-sm w-full">
            <div class="relative w-full rounded-lg border px-4 py-3 text-sm bg-green-50 text-green-800 border-green-200">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">Package "${pkg.name}" updated successfully!</div>
              </div>
            </div>
          </div>
          <script>
          ...
          <div class="fixed top-4 right-4 z-50 max-w-sm w-full">
            <div class="relative w-full rounded-lg border px-4 py-3 text-sm bg-red-50 text-red-800 border-red-200">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">Failed to update package: ${error.message}</div>
              </div>
            </div>
          </div>
        `);
          ...
          <div class="fixed top-4 right-4 z-50 max-w-sm w-full">
            <div class="relative w-full rounded-lg border px-4 py-3 text-sm bg-red-50 text-red-800 border-red-200">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">Package "${name}" has been deleted!</div>
              </div>
            </div>
          </div>
          <script>
          ...
          <div class="fixed top-4 right-4 z-50 max-w-sm w-full">
            <div class="relative w-full rounded-lg border px-4 py-3 text-sm bg-red-50 text-red-800 border-red-200">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">Failed to delete package: ${error.message}</div>
              </div>
            </div>
          </div>
        `);
...
const generatePaginationHtml = (page, limit, total, query, entity) => {
  ...
  const package_type = query.package_type || '';
  const status = query.status || '';
  const params = `limit=${limit}&search=${encodeURIComponent(search)}&package_type=${package_type}&status=${status}`;
...
export const bulkAction = async (req, res) => {
  try {
    ...
    const packageService = serviceFactory.getPackageService();
    const results = [];
    const errors = [];
    ...
    for (const id of ids) {
      try {
        ...
        switch (action) {
          case 'activate':
            result = await packageService.activatePackage(id);
            break;
          case 'deactivate':
            result = await packageService.deactivatePackage(id);
            break;
          case 'delete':
    ...
    logger.info(
      `Bulk ${action} completed for ${results.length} packages, ${errors.length} errors`
    );
    ...
    if (isHtmxRequest(req)) {
      ...
      res.send(`
        ...
        <script>
          htmx.ajax('GET', window.location.pathname + window.location.search, {target: '#packagesTableContainer'});
      ...
      res.json({
        success: true,
        data: { results, errors },
        message: `Bulk ${action} completed for ${results.length} packages`,
      });
    }
  } catch (error) {
