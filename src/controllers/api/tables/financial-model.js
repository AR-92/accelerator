...
export const bulkAction = async (req, res) => {
  try {
    ...
    const financialModelService = serviceFactory.getFinancialModelService();
    const results = [];
    const errors = [];
    ...
    for (const id of ids) {
      try {
        ...
        switch (action) {
          case 'finalize':
            result = await financialModelService.finalizeFinancialModel(id);
            break;
          case 'archive':
            result = await financialModelService.archiveFinancialModel(id);
            break;
          case 'delete':
            await financialModelService.deleteFinancialModel(id);
    ...
    logger.info(
      `Bulk ${action} completed for ${results.length} financial models, ${errors.length} errors`
    );
    ...
    if (isHtmxRequest(req)) {
      ...
      res.send(`
        ...
        <script>
          htmx.ajax('GET', window.location.pathname + window.location.search, {target: '#financial-modelTableContainer'});
      ...
      res.json({
        success: true,
        data: { results, errors },
        message: `Bulk ${action} completed for ${results.length} financial models`,
      });
    }
  } catch (error) {
...
export default function financialModelRoutes(app) {
  ...
  app.post('/api/financial-model/bulk-action', bulkAction);
}
