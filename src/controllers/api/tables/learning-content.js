...
export const bulkAction = async (req, res) => {
  try {
    ...
    const learningContentService = serviceFactory.getLearningContentService();
    const results = [];
    const errors = [];
    ...
    for (const id of ids) {
      try {
        ...
        switch (action) {
          case 'delete':
            await learningContentService.deleteLearningContent(id);
    ...
    logger.info(
      `Bulk ${action} completed for ${results.length} learning content items, ${errors.length} errors`
    );
    ...
    if (isHtmxRequest(req)) {
      ...
      res.send(`
        ...
        <script>
          htmx.ajax('GET', window.location.pathname + window.location.search, {target: '#learning-contentTableContainer'});
      ...
      res.json({
        success: true,
        data: { results, errors },
        message: `Bulk ${action} completed for ${results.length} learning content items`,
      });
    }
  } catch (error) {
...
export default function learningContentRoutes(app) {
  ...
  app.post('/api/learning-content/bulk-action', bulkAction);
