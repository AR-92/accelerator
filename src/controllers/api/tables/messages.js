  ...
  try {
    ...
    for (const id of ids) {
      try {
        ...
        switch (action) {
          case 'markAsRead':
            result = await messageService.markAsReadMessage(id);
            break;
          case 'delete':
    ...
    logger.info(
      `Bulk ${action} completed for ${results.length} messages, ${errors.length} errors`
    );
    ...
    if (isHtmxRequest(req)) {
      ...
      res.send(`
        ...
        <script>
          htmx.ajax('GET', window.location.pathname + window.location.search, {target: '#messagesTableContainer'});
      ...
      res.json({
        success: true,
        data: { results, errors },
        message: `Bulk ${action} completed for ${results.length} messages`,
      });
    }
  } catch (error) {
...
export default function messagesRoutes(app) {
  ...
  app.post('/api/messages/bulk-action', bulkAction);
