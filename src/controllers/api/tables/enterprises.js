...
export const getEnterprises = async (req, res) => {
  try {
    ...
    if (isHtmxRequest(req)) {
      const enterpriseHtml = enterprises.map(enterprise => `
      ...
      const paginationHtml = generatePaginationHtml(pageNum, limitNum, total, req.query, 'enterprises');
      res.send(enterpriseHtml + paginationHtml);
    } else {
      res.json({ success: true, data: enterprises, pagination: { page: pageNum, limit: limitNum, total, totalPages: Math.ceil(total / limitNum) } });
    }
  } catch (error) {
    logger.error('Error fetching enterprises:', error);
    if (isHtmxRequest(req)) {
      res.status(500).send('<p class="text-red-500">Error loading enterprises</p>');
    } else {
