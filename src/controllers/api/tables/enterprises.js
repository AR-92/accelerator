...
export const getEnterprises = async (req, res) => {
  try {
    ...
    const total = count || 0;
    logger.info(`Fetched ${enterprises.length} of ${total} enterprises`);

    if (isHtmxRequest(req)) {
      const enterpriseHtml = enterprises
        .map(
          (enterprise) => `
        <tr class="border-b border-gray-100/40 hover:bg-purple-100 dark:hover:bg-purple-800 transition-colors duration-150">
          <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">${enterprise.name}</td>
          <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${enterprise.type}</td>
          <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${enterprise.status || 'N/A'}</td>
          <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${formatDate(enterprise.created_at)}</td>
        </tr>
      `
        )
        .join('');
      ...
      res.send(enterpriseHtml + paginationHtml);
    } else {
      res.json({
        success: true,
        data: enterprises,
    ...
    if (isHtmxRequest(req)) {
      res
        .status(500)
        .send('<p class="text-red-500">Error loading enterprises</p>');
    } else {
...
export const bulkAction = async (req, res) => {
  try {
    ...
    const enterpriseService = serviceFactory.getEnterpriseService();
    const results = [];
    const errors = [];
    ...
    logger.info(
      `Bulk ${action} completed for ${results.length} enterprises, ${errors.length} errors`
    );
    ...
    if (isHtmxRequest(req)) {
      ...
      res.send(`
        ...
        <script>
          htmx.ajax('GET', window.location.pathname + window.location.search, {target: '#enterprisesTableContainer'});
      ...
      res.json({
        success: true,
        data: { results, errors },
        message: `Bulk ${action} completed for ${results.length} enterprises`,
      });
    }
  } catch (error) {
...
export default function enterprisesRoutes(app) {
  app.get('/api/enterprises', getEnterprises);
  app.post('/api/enterprises/bulk-action', bulkAction);
}
