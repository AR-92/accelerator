...
export const getLearningCategories = async (req, res) => {
  try {
    ...
    if (category_type) filters.push(`category_type: ${category_type}`);
          ...
          <td class="px-6 py-4">
            <div class="relative">
              ...
              <div id="actionMenu-learning-category-${category.id}" class="dropdown-menu hidden absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-10 border border-gray-200">
                <div class="py-1">
                  <a href="/admin/table-pages/learning-categories/${category.id}" class="flex items-center px-4 py-2 text-sm text-black dark:text-white hover:bg-gray-100 transition-colors">
...
// Get single learning category by ID
export const getLearningCategory = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    logger.error('Error fetching learning category:', error);
    res.status(500).json({ success: false, error: error.message });
  }
};

// Create new learning category
export const createLearningCategory = [
  validateLearningCategoryCreation,
  async (req, res) => {
    try {
          ...
          <div class="fixed top-4 right-4 z-50 max-w-sm w-full">
            <div class="relative w-full rounded-lg border px-4 py-3 text-sm bg-green-50 text-green-800 border-green-200">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">Learning category "${category.name}" created successfully!</div>
              </div>
            </div>
          </div>
          <script>
            setTimeout(() => document.querySelector('.fixed').remove(), 5000);
            htmx.trigger('#learningCategoriesTableContainer', 'learningCategoryCreated');
          </script>
        `);
      } else {
          ...
          <div class="fixed top-4 right-4 z-50 max-w-sm w-full">
            <div class="relative w-full rounded-lg border px-4 py-3 text-sm bg-red-50 text-red-800 border-red-200">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">Failed to create learning category: ${error.message}</div>
              </div>
            </div>
          </div>
        `);
...
export const updateLearningCategory = [
  validateLearningCategoryUpdate,
  async (req, res) => {
    try {
      const { id } = req.params;
          ...
          <div class="fixed top-4 right-4 z-50 max-w-sm w-full">
            <div class="relative w-full rounded-lg border px-4 py-3 text-sm bg-green-50 text-green-800 border-green-200">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">Learning category "${category.name}" updated successfully!</div>
              </div>
            </div>
          </div>
          <script>
            setTimeout(() => document.querySelector('.fixed').remove(), 5000);
            htmx.trigger('#learningCategoriesTableContainer', 'learningCategoryUpdated');
          </script>
        `);
      } else {
        res.json({ success: true, data: category });
      ...
      logger.error('Error updating learning category:', error);
      if (isHtmxRequest(req)) {
        res.status(500).send(`
          <div class="fixed top-4 right-4 z-50 max-w-sm w-full">
            <div class="relative w-full rounded-lg border px-4 py-3 text-sm bg-red-50 text-red-800 border-red-200">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">Failed to update learning category: ${error.message}</div>
              </div>
            </div>
          </div>
        `);
...
export const deleteLearningCategory = [
  validateLearningCategoryDeletion,
  async (req, res) => {
    try {
      const { id } = req.params;
          ...
          <div class="fixed top-4 right-4 z-50 max-w-sm w-full">
            <div class="relative w-full rounded-lg border px-4 py-3 text-sm bg-red-50 text-red-800 border-red-200">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">Learning category "${existingCategory.name}" has been deleted!</div>
              </div>
            </div>
          </div>
          <script>
            setTimeout(() => document.querySelector('.fixed').remove(), 5000);
            htmx.trigger('#learningCategoriesTableContainer', 'learningCategoryDeleted');
          </script>
        `);
      } else {
        res.json({
        ...
        res.status(500).send(`
          <div class="fixed top-4 right-4 z-50 max-w-sm w-full">
            <div class="relative w-full rounded-lg border px-4 py-3 text-sm bg-red-50 text-red-800 border-red-200">
              ...
              <div class="flex-1">Failed to delete learning category: ${error.message}</div>
            </div>
          </div>
        `);
      } else {
...
const generatePaginationHtml = (page, limit, total, query) => {
  ...
  const params = `limit=${limit}&search=${encodeURIComponent(search)}&category_type=${category_type}&is_active=${is_active}&is_featured=${is_featured}`;
  ...
  for (let i = startPage; i <= endPage; i++) {
    if (i === page) {
      ...
      html += `<button hx-get="/api/learning-categories?page=${i}&${params}" hx-target="#learningCategoriesTableContainer" class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-9 w-9">${i}</button>`;
    }
  }
  html += `</div>`;
...
export const bulkAction = async (req, res) => {
  try {
    ...
    const learningCategoryService = serviceFactory.getLearningCategoryService();
    const results = [];
    const errors = [];
    ...
    for (const id of ids) {
      try {
        ...
        switch (action) {
          case 'delete':
            await learningCategoryService.deleteLearningCategory(id);
    ...
    logger.info(
      `Bulk ${action} completed for ${results.length} learning categories, ${errors.length} errors`
    );
    ...
    if (isHtmxRequest(req)) {
      ...
      res.send(`
        ...
        <script>
          htmx.ajax('GET', window.location.pathname + window.location.search, {target: '#learning-categoriesTableContainer'});
      ...
      res.json({
        success: true,
        data: { results, errors },
        message: `Bulk ${action} completed for ${results.length} learning categories`,
      });
    }
  } catch (error) {
...
export default function learningCategoriesRoutes(app) {
  app.get('/api/learning-categories', getLearningCategories);
  app.get('/api/learning-categories/:id', getLearningCategory);
  app.post('/api/learning-categories', ...createLearningCategory);
  app.put('/api/learning-categories/:id', ...updateLearningCategory);
  app.post('/api/learning-categories/bulk-action', bulkAction);
  app.delete('/api/learning-categories/:id', ...deleteLearningCategory);
}
