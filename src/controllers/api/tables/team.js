...
export const getTeam = async (req, res) => {
  try {
    ...
    const total = count || 0;
    logger.info(`Fetched ${team.length} of ${total} team members`);
    ...
    if (isHtmxRequest(req)) {
      const teamHtml = team
        ...
        <tr class="border-b border-gray-100/40 hover:bg-purple-100 dark:hover:bg-purple-800 transition-colors duration-150">
          ...
          <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${member.role}</td>
          <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${member.email || 'N/A'}</td>
          <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${formatDate(member.created_at)}</td>
        </tr>
      `
        )
        .join('');
      ...
      res.send(teamHtml + paginationHtml);
    } else {
      res.json({
        success: true,
        data: team,
    ...
    if (isHtmxRequest(req)) {
      res.status(500).send('<p class="text-red-500">Error loading team</p>');
    } else {
...
const generatePaginationHtml = (page, limit, total, query, entity) => {
  ...
  const params = `limit=${limit}&search=${encodeURIComponent(search)}&role=${role}`;
  ...
  try {
    ...
    for (const id of ids) {
      try {
        ...
        switch (action) {
          case 'activate':
            result = await teamService.activateTeam(id);
            break;
          case 'deactivate':
            result = await teamService.deactivateTeam(id);
            break;
          case 'delete':
    ...
    logger.info(
      `Bulk ${action} completed for ${results.length} team members, ${errors.length} errors`
    );
    ...
    if (isHtmxRequest(req)) {
      ...
      res.send(`
        ...
        <script>
          htmx.ajax('GET', window.location.pathname + window.location.search, {target: '#teamTableContainer'});
      ...
      res.json({
        success: true,
        data: { results, errors },
        message: `Bulk ${action} completed for ${results.length} team members`,
      });
    }
  } catch (error) {
...
export default function teamRoutes(app) {
  ...
  app.post('/api/team/bulk-action', bulkAction);
}
