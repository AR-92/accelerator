...
export const getPitchDeck = async (req, res) => {
  try {
    ...
    if (isHtmxRequest(req)) {
        ...
        <tr class="border-b border-gray-100/40 hover:bg-purple-100 dark:hover:bg-purple-800 transition-colors duration-150">
          ...
          <td class="px-6 py-4">
            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
              deck.status === 'active' ? 'bg-green-100 text-green-800' :
              deck.status === 'draft' ? 'bg-yellow-100 text-yellow-800' :
      ...
      const paginationHtml = generatePaginationHtml(pageNum, limitNum, total, req.query, 'pitchdeck');
      res.send(deckHtml + paginationHtml);
    } else {
      res.json({ success: true, data: decks, pagination: { page: pageNum, limit: limitNum, total, totalPages: Math.ceil(total / limitNum) } });
    }
  } catch (error) {
    logger.error('Error fetching pitchdecks:', error);
    if (isHtmxRequest(req)) {
      res.status(500).send('<p class="text-red-500">Error loading pitchdecks</p>');
    } else {
