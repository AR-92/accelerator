...
export const getMarketing = async (req, res) => {
  try {
    ...
    const { data: marketing, error, count } = await query;

    if (error) throw error;

    const total = count || 0;
    logger.info(`Fetched ${marketing.length} of ${total} marketing items`);
    ...
    if (isHtmxRequest(req)) {
      ...
      res.send(marketingHtml + paginationHtml);
    } else {
      res.json({
        success: true,
        data: marketing,
    ...
    logger.error('Error fetching marketing:', error);
    if (isHtmxRequest(req)) {
      res
        .status(500)
        .send('<p class="text-red-500">Error loading marketing</p>');
    } else {
...
export const bulkAction = async (req, res) => {
  try {
    ...
    const marketingService = serviceFactory.getMarketingService();
    const results = [];
    const errors = [];
    ...
    for (const id of ids) {
      try {
        ...
        switch (action) {
          case 'activate':
            result = await marketingService.activateMarketing(id);
            break;
          case 'archive':
            result = await marketingService.archiveMarketing(id);
            break;
          case 'delete':
    ...
    logger.info(
      `Bulk ${action} completed for ${results.length} marketing items, ${errors.length} errors`
    );
    ...
    if (isHtmxRequest(req)) {
      ...
      res.send(`
        ...
        <script>
          htmx.ajax('GET', window.location.pathname + window.location.search, {target: '#marketingTableContainer'});
      ...
      res.json({
        success: true,
        data: { results, errors },
        message: `Bulk ${action} completed for ${results.length} marketing items`,
      });
    }
  } catch (error) {
...
export default function marketingRoutes(app) {
  app.get('/api/marketing', getMarketing);
  app.post('/api/marketing/bulk-action', bulkAction);
}
