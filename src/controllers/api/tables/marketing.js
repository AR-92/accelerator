...
export const getMarketing = async (req, res) => {
  try {
    ...
    if (isHtmxRequest(req)) {
      const marketingHtml = marketing.map(item => `
      ...
      const paginationHtml = generatePaginationHtml(pageNum, limitNum, total, req.query, 'marketing');
      res.send(marketingHtml + paginationHtml);
    } else {
      res.json({ success: true, data: marketing, pagination: { page: pageNum, limit: limitNum, total, totalPages: Math.ceil(total / limitNum) } });
    }
  } catch (error) {
    logger.error('Error fetching marketing:', error);
    if (isHtmxRequest(req)) {
      res.status(500).send('<p class="text-red-500">Error loading marketing</p>');
    } else {
