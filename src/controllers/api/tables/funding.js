...
export const getFunding = async (req, res) => {
  try {
    ...
    if (isHtmxRequest(req)) {
      const fundingHtml = fundings.map(fund => `
      ...
      res.json({ success: true, data: fundings, pagination: { page: pageNum, limit: limitNum, total, totalPages: Math.ceil(total / limitNum) } });
    }
  } catch (error) {
    logger.error('Error fetching fundings:', error);
    if (isHtmxRequest(req)) {
      res.status(500).send('<p class="text-red-500">Error loading fundings</p>');
    } else {
