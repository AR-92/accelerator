...
export const getFunding = async (req, res) => {
  try {
    ...
    const total = count || 0;
    logger.info(`Fetched ${fundings.length} of ${total} fundings`);

    if (isHtmxRequest(req)) {
      const fundingHtml = fundings
        .map(
          (fund) => `
        <tr class="border-b border-gray-100/40 hover:bg-purple-100 dark:hover:bg-purple-800 transition-colors duration-150">
          <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">${fund.name}</td>
          <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${fund.amount || 'N/A'}</td>
          <td class="px-6 py-4">
          ...
          <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${formatDate(fund.created_at)}</td>
        </tr>
      `
        )
        .join('');
    ...
    if (isHtmxRequest(req)) {
      res
        .status(500)
        .send('<p class="text-red-500">Error loading fundings</p>');
    } else {
...
export const bulkAction = async (req, res) => {
  try {
    ...
    const fundingService = serviceFactory.getFundingService();
    const results = [];
    const errors = [];
    ...
    for (const id of ids) {
      try {
        ...
        switch (action) {
          case 'fund':
            result = await fundingService.fundFunding(id);
            break;
          case 'close':
            result = await fundingService.closeFunding(id);
            break;
          case 'delete':
            await fundingService.deleteFunding(id);
    ...
    logger.info(
      `Bulk ${action} completed for ${results.length} funding items, ${errors.length} errors`
    );
    ...
    if (isHtmxRequest(req)) {
      ...
      res.send(`
        ...
        <script>
          htmx.ajax('GET', window.location.pathname + window.location.search, {target: '#fundingTableContainer'});
      ...
      res.json({
        success: true,
        data: { results, errors },
        message: `Bulk ${action} completed for ${results.length} funding items`,
      });
    }
  } catch (error) {
...
export default function fundingRoutes(app) {
  app.get('/api/funding', getFunding);
  app.post('/api/funding/bulk-action', bulkAction);
}
