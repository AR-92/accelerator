...
export const getRewards = async (req, res) => {
  try {
    ...
    if (isHtmxRequest(req)) {
      const rewardHtml = rewards.map(reward => `
        <tr class="border-b border-gray-100/40 hover:bg-purple-100 dark:hover:bg-purple-800 transition-colors duration-150">
          <td class="px-6 py-4">
            <div class="flex items-center">
              ...
              <div class="ml-4">
                ...
                <div class="text-sm text-gray-500">${reward.description?.substring(0, 50)}${reward.description?.length > 50 ? '...' : ''}</div>
              </div>
            </div>
          </td>
          <td class="px-6 py-4">
          ...
          <td class="px-6 py-4">
            <div class="relative">
              <button onclick="toggleActionMenu('reward', ${reward.id})" class="p-2 rounded-full hover:bg-gray-100 text-gray-500 hover:text-black dark:text-white transition-colors">
              ...
              <div id="actionMenu-reward-${reward.id}" class="dropdown-menu hidden absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-10 border border-gray-200">
                <div class="py-1">
                  <a href="/admin/table-pages/rewards/${reward.id}" class="flex items-center px-4 py-2 text-sm text-black dark:text-white hover:bg-gray-100 transition-colors">
                  ...
                  <button onclick="editReward(${reward.id})" class="flex items-center w-full px-4 py-2 text-sm text-black dark:text-white hover:bg-gray-100 transition-colors">
                  ...
                  <button onclick="deleteReward(${reward.id}, '${reward.title}')" class="flex items-center w-full px-4 py-2 text-sm text-red-600 hover:bg-red-50 transition-colors">
    ...
    if (isHtmxRequest(req)) {
      res.status(500).send('<p class="text-red-500">Error loading rewards</p>');
    } else {
          ...
          <div class="fixed top-4 right-4 z-50 max-w-sm w-full">
            <div class="relative w-full rounded-lg border px-4 py-3 text-sm bg-green-50 text-green-800 border-green-200">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">Reward "${reward.title}" created successfully!</div>
              </div>
            </div>
          </div>
          <script>
            setTimeout(() => document.querySelector('.fixed').remove(), 5000);
            htmx.trigger('#rewardsTableContainer', 'rewardCreated');
          </script>
        `);
      } else {
          ...
          <div class="fixed top-4 right-4 z-50 max-w-sm w-full">
            <div class="relative w-full rounded-lg border px-4 py-3 text-sm bg-red-50 text-red-800 border-red-200">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">Failed to create reward: ${error.message}</div>
              </div>
            </div>
          </div>
        `);
          ...
          <div class="fixed top-4 right-4 z-50 max-w-sm w-full">
            <div class="relative w-full rounded-lg border px-4 py-3 text-sm bg-green-50 text-green-800 border-green-200">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">Reward "${reward.title}" updated successfully!</div>
              </div>
            </div>
          </div>
          <script>
            setTimeout(() => document.querySelector('.fixed').remove(), 5000);
            htmx.trigger('#rewardsTableContainer', 'rewardUpdated');
          </script>
        `);
      } else {
        res.json({ success: true, data: reward });
          ...
          <div class="fixed top-4 right-4 z-50 max-w-sm w-full">
            <div class="relative w-full rounded-lg border px-4 py-3 text-sm bg-red-50 text-red-800 border-red-200">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">Failed to update reward: ${error.message}</div>
              </div>
            </div>
          </div>
        `);
          ...
          <div class="fixed top-4 right-4 z-50 max-w-sm w-full">
            <div class="relative w-full rounded-lg border px-4 py-3 text-sm bg-red-50 text-red-800 border-red-200">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">Reward "${existingReward.title}" has been deleted!</div>
              </div>
            </div>
          </div>
          <script>
            setTimeout(() => document.querySelector('.fixed').remove(), 5000);
            htmx.trigger('#rewardsTableContainer', 'rewardDeleted');
          </script>
        `);
      } else {
        ...
        res.status(500).send(`
          <div class="fixed top-4 right-4 z-50 max-w-sm w-full">
            <div class="relative w-full rounded-lg border px-4 py-3 text-sm bg-red-50 text-red-800 border-red-200">
              ...
              <div class="flex-1">Failed to delete reward: ${error.message}</div>
            </div>
          </div>
        `);
      } else {
...
const generatePaginationHtml = (page, limit, total, query) => {
  ...
  for (let i = startPage; i <= endPage; i++) {
    if (i === page) {
      ...
      html += `<button hx-get="/api/rewards?page=${i}&${params}" hx-target="#rewardsTableContainer" class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-9 w-9">${i}</button>`;
    }
  }
  html += `</div>`;
