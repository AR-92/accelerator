  ...
  try {
    ...
    for (const id of ids) {
      try {
        ...
        switch (action) {
          case 'archive':
            result = await contentService.archiveContent(id);
            break;
          case 'delete':
    ...
    logger.info(
      `Bulk ${action} completed for ${results.length} content items, ${errors.length} errors`
    );
    ...
    if (isHtmxRequest(req)) {
      ...
      res.send(`
        ...
        <script>
          htmx.ajax('GET', window.location.pathname + window.location.search, {target: '#contentTableContainer'});
      ...
      res.json({
        success: true,
        data: { results, errors },
        message: `Bulk ${action} completed for ${results.length} content items`,
      });
    }
  } catch (error) {
...
export default function contentRoutes(app) {
  ...
  app.post('/api/content/bulk-action', bulkAction);
