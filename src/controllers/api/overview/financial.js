...
export const getFinancial = async (req, res) => {
  try {
    ...
    const { data: financials, error, count } = await query;

    if (error) throw error;

    const total = count || 0;
    logger.info(`Fetched ${financials.length} of ${total} financial items`);

    if (isHtmxRequest(req)) {
      const financialHtml = financials.map(item => `
        <tr class="border-b border-gray-100/40 hover:bg-purple-100 dark:hover:bg-purple-800 transition-colors duration-150">
          ...
          <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${item.amount || 'N/A'}</td>
          <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${formatDate(item.created_at)}</td>
        </tr>
      `).join('');

      const paginationHtml = generatePaginationHtml(pageNum, limitNum, total, req.query, 'financial');
      res.send(financialHtml + paginationHtml);
    } else {
      res.json({ success: true, data: financials, pagination: { page: pageNum, limit: limitNum, total, totalPages: Math.ceil(total / limitNum) } });
    }
  } catch (error) {
    logger.error('Error fetching financial:', error);
    if (isHtmxRequest(req)) {
      res.status(500).send('<p class="text-red-500">Error loading financial</p>');
    } else {
...
export default function financialRoutes(app) {
  app.get('/api/financial', getFinancial);
}