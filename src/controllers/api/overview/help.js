...
export const getHelp = async (req, res) => {
  try {
    ...
    const { data: helpItems, error, count } = await query;

    if (error) throw error;

    const total = count || 0;
    logger.info(`Fetched ${helpItems.length} of ${total} help items`);
    ...
    if (isHtmxRequest(req)) {
      const helpHtml = helpItems.map(item => `
        <tr class="border-b border-gray-100/40 hover:bg-purple-100 dark:hover:bg-purple-800 transition-colors duration-150">
          ...
          <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${item.category}</td>
          <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${formatDate(item.created_at)}</td>
        </tr>
      `).join('');

      const paginationHtml = generatePaginationHtml(pageNum, limitNum, total, req.query, 'help');
      res.send(helpHtml + paginationHtml);
    } else {
      res.json({ success: true, data: helpItems, pagination: { page: pageNum, limit: limitNum, total, totalPages: Math.ceil(total / limitNum) } });
    }
  } catch (error) {
    logger.error('Error fetching help:', error);
    if (isHtmxRequest(req)) {
      res.status(500).send('<p class="text-red-500">Error loading help</p>');
    } else {
...
const generatePaginationHtml = (page, limit, total, query, entity) => {
  ...
  const params = `limit=${limit}&search=${encodeURIComponent(search)}&category=${category}`;
