...
export const getTables = async (req, res) => {
  try {
    ...
    const total = count || 0;
    logger.info(`Fetched ${tables.length} of ${total} tables`);

    if (isHtmxRequest(req)) {
      const tableHtml = tables.map(table => `
        <tr class="border-b border-gray-100/40 hover:bg-purple-100 dark:hover:bg-purple-800 transition-colors duration-150">
          <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">${table.name}</td>
          <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${table.description || 'N/A'}</td>
          <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${table.record_count || 0}</td>
          <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${formatDate(table.created_at)}</td>
        </tr>
      `).join('');

      const paginationHtml = generatePaginationHtml(pageNum, limitNum, total, req.query, 'tables');
      res.send(tableHtml + paginationHtml);
    } else {
      res.json({ success: true, data: tables, pagination: { page: pageNum, limit: limitNum, total, totalPages: Math.ceil(total / limitNum) } });
    }
  } catch (error) {
    logger.error('Error fetching tables:', error);
    if (isHtmxRequest(req)) {
      res.status(500).send('<p class="text-red-500">Error loading tables</p>');
    } else {
...
const generatePaginationHtml = (page, limit, total, query, entity) => {
  ...
  const params = `limit=${limit}&search=${encodeURIComponent(search)}`;
...
export default function tablesRoutes(app) {
