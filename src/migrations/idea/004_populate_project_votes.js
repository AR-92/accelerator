/**
 * Migration: Populate votes table with sample data using project_id
 * Creates sample votes connecting users to projects
 */
...
module.exports = {
  up: async (db) => {
    try {
      ...
      if (!investorUser || !businessUser || !developerUser || !mentorUser || !designerUser || !studentUser) {
        console.log('Required users do not exist yet, skipping votes population');
        return true;
      }

      if (!healthTrackerProject || !aiCustomerProject || !fileStorageProject || !packagingProject || !paymentGatewayProject) {
        console.log('Required projects do not exist yet, skipping votes population');
        return true;
      }
  ...
  down: async (db) => {
    try {
      ...
      if (!investorUser || !businessUser || !developerUser || !mentorUser || !designerUser || !studentUser) {
        console.log('Required users do not exist, no votes to remove');
        return true;
      }
